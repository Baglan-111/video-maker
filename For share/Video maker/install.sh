...
# Remotion рендерит через собственный Chrome и качает его при первом рендере
# (около 190 МБ). Делаем это сейчас: иначе первый же ролик встаёт на несколько
# минут без объяснений, а без интернета просто падает.
say "Качаю браузер для рендера (около 190 МБ, один раз) …"
npx remotion browser ensure || say "ПРЕДУПРЕЖДЕНИЕ: браузер не скачался, Remotion сделает это при первом рендере."
